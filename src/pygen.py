...
MODEL_FOLDER_ROUTE = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] is not None else "es.beyond.base"
RESULT_PATH = sys.argv[3] if len(sys.argv) > 3 and sys.argv[3] is not None else os.getcwd()
...
def create_directories(result_path):
	src_path = "{}/{}".format(result_path, lower_case_first_char(MODEL_NAME))  # foo
...
def create_dao_files(dao_interfaces_path, dao_path):
	try:
		foo_dao_interface = open("{}/I{}DAO.java".format(dao_interfaces_path, upper_case_first_char(MODEL_NAME)), "w+")
		foo_dao_interface.write(generate_dao_interface(MODEL_FOLDER_ROUTE, MODEL_NAME))

		foo_generic_dao_interface = open("{}/I{}GenericoDAO.java".format(dao_interfaces_path, upper_case_first_char(MODEL_NAME)),
										 "w+")
		foo_generic_dao_interface.write(generate_generic_dao_interface(MODEL_FOLDER_ROUTE, MODEL_NAME))

		foo_dao = open("{}/{}DAO.java".format(dao_path, upper_case_first_char(MODEL_NAME)), "w+")
		foo_dao.write(generate_foo_dao(MODEL_FOLDER_ROUTE, MODEL_NAME))

		foo_generic_dao = open("{}/{}GenericoDAO.java".format(dao_path, upper_case_first_char(MODEL_NAME)), "w+")
...
def create_model_files(model_path):
	try:
		...
		foo_model = open("{}/{}.java".format(model_path, upper_case_first_char(MODEL_NAME)), "w+")
...
def create_service_files(service_interfaces_path, service_path):
	try:
		foo_service_interface = open("{}/I{}Service.java".format(service_interfaces_path, upper_case_first_char(MODEL_NAME)),
									 "w+")
		foo_service_interface.write(generate_service_interface(MODEL_FOLDER_ROUTE, MODEL_NAME))

		foo_service = open("{}/{}Service.java".format(service_path, upper_case_first_char(MODEL_NAME)), "w+")
...
dao_path, dao_interfaces_path, model_path, service_interfaces_path, service_path = create_directories(RESULT_PATH)
